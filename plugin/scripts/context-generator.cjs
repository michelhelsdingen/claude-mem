...
>>>>>>> 409b6012 (feat: add CLAUDE_MEM_PROJECT setting for remote client project override)
...
=======
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",l={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},m=new G;var Rt={};function Ot(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,ce.fileURLToPath)(Rt.url))}var Ct=Ot(),I=y.get("CLAUDE_MEM_DATA_DIR"),W=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,de.homedir)(),".claude"),Wt=(0,h.join)(I,"archives"),Yt=(0,h.join)(I,"logs"),Vt=(0,h.join)(I,"trash"),qt=(0,h.join)(I,"backups"),Kt=(0,h.join)(I,"modes"),Jt=(0,h.join)(I,"settings.json"),me=(0,h.join)(I,"claude-mem.db"),zt=(0,h.join)(I,"vector-db"),Qt=(0,h.join)(W,"settings.json"),Zt=(0,h.join)(W,"commands"),es=(0,h.join)(W,"CLAUDE.md");function ue(r){(0,pe.mkdirSync)(r,{recursive:!0})}function le(){return(0,h.join)(Ct,"..")}var U=class{db;constructor(e=me){e!==":memory:"&&ue(I),this.db=new _e.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn()}initializeSchema(){this.db.run(`
>>>>>>> 409b6012 (feat: add CLAUDE_MEM_PROJECT setting for remote client project override)
