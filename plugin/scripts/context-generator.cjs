...
`+JSON.stringify(o,null,2):_=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:g,correlationId:h,...l}=n;Object.keys(l).length>0&&(u=` {${Object.entries(l).map(([f,R])=>`${f}=${R}`).join(", ")}}`)}let T=`[${i}] [${a}] [${d}] ${m}${s}${u}${_}`;if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),_=m?`${m[1].split("/").pop()}:${m[2]}`:"unknown",u={...s,location:_};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},c=new W;var Rt={};function ht(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,me.fileURLToPath)(Rt.url))}var Ot=ht(),N=I.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,de.homedir)(),".claude"),Ht=(0,b.join)(y,"plugins","marketplaces","thedotmack"),Wt=(0,b.join)(N,"archives"),Vt=(0,b.join)(N,"logs"),Yt=(0,b.join)(N,"trash"),qt=(0,b.join)(N,"backups"),Kt=(0,b.join)(N,"modes"),Jt=(0,b.join)(N,"settings.json"),ce=(0,b.join)(N,"claude-mem.db"),zt=(0,b.join)(N,"vector-db"),Qt=(0,b.join)(N,"observer-sessions"),Zt=(0,b.join)(y,"settings.json"),es=(0,b.join)(y,"commands"),ts=(0,b.join)(y,"CLAUDE.md");function _e(r){(0,pe.mkdirSync)(r,{recursive:!0})}function ue(){return(0,b.join)(Ot,"..")}var $=class{db;constructor(e=ce){e!==":memory:"&&_e(N),this.db=new le.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys()}initializeSchema(){this.db.run(`
    ...
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,d,a);return{id:Number(_.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),_=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(_.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),m=new Date(d).toISOString();return this.db.transaction(()=>{let u=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let h=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,m,d);u.push(Number(h.lastInsertRowid))}let E=null;if(n){let h=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,m,d);E=Number(h.lastInsertRowid)}return{observationIds:u,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,m){let _=m??Date.now(),u=new Date(_).toISOString();return this.db.transaction(()=>{let E=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ...
      `).run(_,o),{observationIds:E,summaryId:h,createdAtEpoch:_}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),m=[...e],_=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&m.push(o),this.db.prepare(`
    ...
    `).all(...m)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,m;if(e!==null){let l=`
      ...
      `,S=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let f=this.db.prepare(l).all(t,...a,s),R=this.db.prepare(S).all(t,...a,n+1);if(f.length===0&&R.length===0)return{observations:[],sessions:[],prompts:[]};d=f.length>0?f[f.length-1].created_at_epoch:t,m=R.length>0?R[R.length-1].created_at_epoch:t}catch(f){return c.error("DB","Error getting boundary timestamps",void 0,{error:f,project:o}),{observations:[],sessions:[],prompts:[]}}}let _=`
      SELECT *
      FROM observations
...
`}function et(r,e,t,s){let n=[];return s?n.push(...je(r)):n.push(...Ae(r)),s?n.push(...Ge()):n.push(...ye()),s?n.push(...Be()):n.push(...ve()),s?n.push(...He()):n.push(...Le()),P(t)&&(s?n.push(...We(e,t)):n.push(...Me(e,t))),n}var ee=L(require("path"),1);function B(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return c.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function st(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function rt(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function nt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function tt(r,e){return ee.default.isAbsolute(r)?ee.default.relative(e,r):r}function ot(r,e,t){let s=B(r);if(s.length>0)return tt(s[0],e);if(t){let n=B(t);if(n.length>0)return tt(n[0],e)}return"General"}function At(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=nt(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function yt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?B(r.facts).join(`
`):null}function vt(r,e,t,s,n,o){let i=[];o?i.push(...Ve(r)):i.push(...De(r));let a=null,d="",m=!1;for(let _ of e)if(_.type==="summary"){m&&(i.push(""),m=!1,a=null,d="");let u=_.data,T=st(u.displayTime);o?i.push(...Je(u,T)):i.push(...we(u,T))}else{let u=_.data,T=ot(u.files_modified,n,u.files_read),E=rt(u.created_at),g=E!==d,h=g?E:"";d=E;let l=t.has(u.id);if(T!==a&&(m&&i.push(""),o?i.push(...Ye(T)):i.push(...xe(T)),a=T,m=!0),l){let S=yt(u,s);o?i.push(...Ke(u,E,g,S,s)):(m&&!o&&(i.push(""),m=!1),i.push(...Ue(u,h,S,s)),a=null)}else o?i.push(qe(u,E,g,s)):i.push(ke(u,h,s))}return m&&i.push(""),i}function it(r,e,t,s,n){let o=[],i=At(r);for(let[a,d]of i)o.push(...vt(a,d,e,t,s,n));return o}function at(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function dt(r,e){let t=[];return e?(t.push(...k("Investigated",r.investigated,p.blue)),t.push(...k("Learned",r.learned,p.yellow)),t.push(...k("Completed",r.completed,p.green)),t.push(...k("Next Steps",r.next_steps,p.magenta))):(t.push(...x("Investigated",r.investigated)),t.push(...x("Learned",r.learned)),t.push(...x("Completed",r.completed)),t.push(...x("Next Steps",r.next_steps))),t}function pt(r,e){return e?ze(r):$e(r)}function mt(r,e,t){return!P(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?Qe(r.totalDiscoveryTokens,r.totalReadTokens):Fe(r.totalDiscoveryTokens,r.totalReadTokens)}var Lt=ct.default.join((0,_t.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Mt(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,ut.unlinkSync)(Lt)}catch(e){c.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return c.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Dt(r,e){return e?Ze(r):Pe(r)}function xt(r,e,t,s,n,o,i){let a=[],d=K(e);a.push(...et(r,d,s,i));let m=t.slice(0,s.sessionCount),_=Ne(m,t),u=Z(e,_),T=Ce(e,s.fullObservationCount);a.push(...it(u,T,s,n,i));let E=t[0],g=e[0];at(s,E,g)&&a.push(...dt(E,i));let h=Q(e,s,o,n);return a.push(...pt(h,i)),a.push(...mt(d,s,i)),a.join(`
`).trimEnd()}async function te(r,e=!1){let t=V(),s=r?.cwd??process.cwd(),n=ge(s),o=r?.projects||[n],i=Mt();if(!i)return"";try{let a=o.length>1?Oe(i,o,t):J(i,n,t),d=o.length>1?Re(i,o,t):z(i,n,t);return a.length===0&&d.length===0?Dt(n,e):xt(n,a,d,t,s,r?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
